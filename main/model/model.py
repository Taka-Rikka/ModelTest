...
from .gat import GAT, HybridResidualGAT, GatedResidualDiffusionGAT
...
class GINEncoder(nn.Module):
    """
    Graph encoder used by the DeepIM variant in this project.

    The historical class name is preserved for compatibility, while the actual
    backbone has been replaced with a gated residual diffusion attention
    module so the modified method is easier to describe as an original model.
    """
    def __init__(self, num_layers, num_mlp_layers, input_dim, hidden_dim, output_dim, 
        ...
        self.gin = GatedResidualDiffusionGAT(
            nfeat=input_dim,
            nhid=hidden_dim,
            nclass=output_dim,
    ...
    def forward(self, batch_graph):
        ...
        x = g.node_features
        adj = g.adj
        node_embs = self.gin(x, adj)
        mean = self.linear_out_mu(node_embs)
        log_var = self.linear_out_logvar(node_embs)
        return mean, log_var


class GRDGATEncoder(GINEncoder):
    """
    Paper-oriented alias:
    GRDGAT = Gated Residual Diffusion Graph Attention Network.
    """
    pass
